...
{/* 
          ...
          <div className="mt-8 grid items-center gap-10 lg:grid-cols-[0.95fr_1.05fr]">
            <motion.div style={{ y: heroTextY }}>
              ...
              <h1 className="mt-8 font-display text-[3.8rem] leading-[0.9] tracking-[-0.04em] text-[#2b1c0e] sm:text-[5.2rem] lg:text-[6.8rem]">
                {product.name}
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-[#3f2a15]/82">{product.description}</p>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {product.notes.map((note) => (
                  <span
                    key={note}
                    className="rounded-full border border-[#6d5130]/15 bg-[#fff8ea]/45 px-3 py-2 text-xs uppercase tracking-[0.18em] text-[#3f2a15]/82"
                  >
                    {note}
                  </span>
                ))}
      ...
      <motion.div style={{ y: heroImageY, rotate: heroImageRotate }}>
        <Reveal direction="right">
          ...
          >
            <p className="text-[11px] uppercase tracking-[0.28em] text-[#6e4a20]">Parallax detail</p>
            <p className="mt-2 font-display text-2xl text-[#23160c]">{product.callout}</p>
          </motion.div>
        </Reveal>
      </motion.div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Reveal direction="left">
            <div className="dudley-card p-8">
              ...
              <h2 className="mt-4 font-display text-4xl text-[#23160c]">{product.callout}</h2>
              <p className="mt-4 text-base leading-7 text-[#3f2a15]/82">
          ...
          {product.highlights.map((item, index) => (
            <Reveal key={item.title} direction={index % 2 === 0 ? 'up' : 'right'} delay={index * 0.08}>
              <div className="dudley-card dudley-card-hover p-8">
                ...
                <h3 className="mt-5 font-display text-3xl text-[#23160c]">{item.title}</h3>
                <p className="mt-3 text-base leading-7 text-[#3f2a15]/82">{item.text}</p>
              </div>
            </Reveal>
          ))}
        </div>
    ...
    <div className="dudley-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-white/10 px-6 py-5">
        <div>
          ...
          <h2 className="mt-3 font-display text-4xl text-[#23160c]">Updated product imagery</h2>
        </div>
      </div>
      <div className="grid gap-6 p-6 sm:grid-cols-2">
        {product.gallery.map((image, index) => (
          <Reveal key={`${image}-${index}`} direction={index % 2 === 0 ? 'up' : 'right'} delay={index * 0.08}>
            <div 
              className="dudley-media-panel h-[460px] sm:h-[500px] lg:h-[540px] cursor-pointer" // Increased heights
        ...
        <div className="dudley-card overflow-hidden p-4">
          <div className="grid gap-6">
            {product.detailImage && (
              <Reveal direction="up">
                <div 
                  className="dudley-media-panel h-[460px] sm:h-[500px] lg:h-[840px] cursor-pointer" // Increased heights
  ...
  <section className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-10 flex flex-col items-center">
      ...
      <div className="grid gap-6 w-full max-w-4xl">
        {product.extraImages.map((image, index) => (
          <Reveal key={`${image}-${index}`} direction={index % 2 === 0 ? 'left' : 'right'} delay={index * 0.08}>
            <div 
              className="dudley-card dudley-media-panel h-[500px] lg:h-[900px] overflow-hidden cursor-pointer flex items-center justify-center"
